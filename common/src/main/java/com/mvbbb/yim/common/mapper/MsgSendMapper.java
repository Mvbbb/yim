...
public interface MsgSendMapper extends BaseMapper<MsgSend> {
    ...
    @Select("select * from yim_msg_send where (to_uid = #{userId} and from_uid = #{friendId}) or (to_uid = #{friendId} and from_uid = #{userId}) order by `timestamp` desc limit ${from},${limit}")
